...
- `-h` now ends with a dedicated rutracker section + examples via `_HELP_EPILOG` in cli.py (`epilog=` + `RawDescriptionHelpFormatter`): preset table with forum counts, -F/-P/-L explanations, the 500-cap forum-major tip, and 11 examples (movies 'dune', tv 'severance', 2026+2160p, hi-res browse, DSD+--codec dsd, -F pair, -F browse pattern, UHD/HD movie browses, new-releases combo 252+1950+1803, Atmos 1163 + digitized rock 1756). `-F` help + tip now state browse mode (no query) explicitly. HowToUse.md §4 mirrors these with HD/UHD + hi-res + digitization browse examples.
