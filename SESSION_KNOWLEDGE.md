...
- Presets now 5: `hi-res` (renamed from `hires`), `digitizations` (16), `dsd` (32), `movies` (106), `tv-series` (78). Superseded id lists removed — the source of truth is `FORUM_PRESETS`/`FORUM_DIRECTORY` in rutracker.py (rebuilt from the live tree; original 30/40-id lists are git history). Discussion-only forums excluded (`f=26` etc.); `f=1417` turned out to be "Breaking Bad" (id recycled), not "Foreign series DVD".
...
- Browse-mode behavior (live-verified): repeated `f=` ORs forums correctly (server count 500 = hard cap on the movies scope), BUT pages arrive forum-major — 50/50 rows from a single forum (movies p1 = f=2339 art-house HD; tv pages = Thai/Indonesia/Singapore forum). Not a bulk-upload artifact (22-23 distinct uploaders, scattered dates). `s`/`o` sort params are IGNORED in browse mode (probe6: seeders column unordered despite s=2&o=2). Music presets under the cap (hi-res: 390) mix normally on page 1. CORRECTED rule (same day, superseding the earlier "pair preset with term"): **never pair a query with a big preset — see addendum "query × multi-f= loss".**
...
- `-h` now ends with a dedicated rutracker section + examples via `_HELP_EPILOG` in cli.py (`epilog=` + `RawDescriptionHelpFormatter`): preset table with forum counts, -F/-P/-L explanations, the search-shape tip (query+1-2 forums; browse presets), and 11 examples (dune -F 1457 -q 2160p, severance -F 266, browse -F 1457 -P 2 -q 2160p, hi-res browse, DSD+--codec dsd, -F pair, -F browse pattern, UHD/HD movie browses, new-releases combo 252+1950+1803, Atmos 1163 + digitized rock 1756). `-F` help + tip now state browse mode (no query) explicitly. HowToUse.md §4 mirrors these with HD/UHD + hi-res + digitization browse examples.
...
- **Query × multi-`f=` loss (live-verified, explains the "0 results" reports):** `nm=` combined with MANY `f=` silently loses matches server-side. nm=dune: single f=1457 → 8 correct rows; site-wide → 50; 30-forum movies preset → 6 rows (only the 1984 Lynch film, wrong); first-15 forums → 0. 32-forum 'DSD' -C dsd degraded but usable (16 rows). 2-forum 'flac' f=1755+1756 → 50 (fine). Rule: **query + 1-2 `-F` ids, or browse-only presets.** Old docs tip "pair the preset with a search term" was backwards — fixed in epilog/HowToUse/this file.
...
- **US & Canada TV forums were MISSING from FORUM_DIRECTORY** — RESOLVED (see tree-walk rebuild below). Historical: real leaf ids f=235 (standard), f=266 (HD), f=1669 (UHD), live-extracted from td[2] viewforum links. tv-series previously couldn't find US shows ('severance' in 842/119/2366/1803 → all 0; in f=266/1669 → 6 rows each).
- **Silent-exit paths in `rutracker.search()`** (lines ~481-522): `_login()` False → [], HTTP 403/429/503 → [], bare except → swallowed. CLI prints no failure line, so empty tables look like success. Transient cookie/clearance rotation produced a full all-zero batch once; raw curl_cffi probe (rt_probe7 pattern: status + row count + cf-marker scan) distinguishes "site healthy" from "scraper path broken".
- Re-verified working examples (all live, same day): `dune -F 1457 -q 2160p` → 8; `severance -F 266` → 6; `severance -F 1669` → 6; browse `-F 1457 -P 2 -q 2160p` → 100; site-wide `severance` → 50; `'DSD' -C dsd --codec dsd` → 16; `'flac' -F 1755 -F 1756` → 50; `'pink floyd' -F 1756` → 50 (HowToUse's old `pink floyd -C hi-res` → 0 twice — preset+query unreliable even at 16 forums, while the 32-f DSD preset returned 16; server behavior is inconsistent, treat preset+query as "do not use"). Epilog + HowToUse examples replaced accordingly (movies/TV/music/year examples now use -F shapes).
- Caveat: rutracker occasionally returns 0 for a shape that works minutes later (whole batch once came back empty incl. proven queries; `pink floyd -F 1755` → 0 once, site-wide 50 right after). If a known-good example returns 0, re-run once before concluding.

## Tree-walk rebuild of movies/tv-series presets + FORUM_DIRECTORY (2026-09-06, live)

- Method: walked the live branch tree (general subagent, 15 requests, 241 forums mapped → /tmp/opencode/tree_walk.json). KEY discovery: **plain index.php TRUNCATES subforum lists (f=189 showed 1 of 26) — `index.php?c=N` category pages are the complete source**; f=235/266/1669 appear nowhere on plain index.php. Viewforum pages list child forums; recurse into non-leaves. Archives (190/665/931) are hidden on category pages but exist on viewforum.
- **rutracker RECYCLES dead forum ids** — old ids silently point at different forums now: 32 (was "Old Russian series" label → actually Обсуждение), 91 (→ single show "Я знаю твои секреты"), 1417 (→ Breaking Bad), 1214 (→ Australia & NZ), 1359 (→ Web series), 704 (→ Turkey), 781 (→ multi-country co-pro), 1301 (→ India), 1539 (→ LatAm subtitled), 1574 (→ LatAm dubbed), 915 (→ Korean), 1242 (→ Korean HD), 2102 (→ Asian clips), 2412 (→ Thailand/Indonesia/Singapore). Dropped as dead/repurposed-elsewhere: 32, 1117, 823, 1606, 1938, 2103, 2104, 1493 (1493 is now "Спектакли без перевода" under Theater → moved to movies preset).
- Result: movies 30 → 106 ids (added f=7 year/genre subtree, Мультфильмы, Аниме, DVD/HD/UHD country leaves, 3D, theater 513-group), tv-series 40 → 78 (added US & Canada 235/266/1669, all foreign HD/UHD country leaves 193/625/825/1248/1288/1459/1463/1690/173/1949/2393, show forums GoT/Sopranos/Supernatural/X-Files/TwinPeaks/TBBT/Suits/WalkingDead + HD twins, Russian single-show forums, Asian UHD 820, Japanese 1939). hi-res/digitizations verified 16/16 exact match — untouched.
...
- Epilog counts synced (movies 106 / tv-series 78); -C label updated to mention US & Canada. Lookup flow for future edits: regenerate via /tmp/opencode/gen_presets.py against a fresh tree_walk.json, splice between `FORUM_PRESETS["movies"]` and `FORUM_NAMES`.

## No-keyword browse + YTS pagination + eztvx beta API (2026-09-06, live-verified)

- `BaseScraper.supports_browse` flag (False default; yts+eztvx True; rutracker stays False — CLI checks its -C/-F separately). CLI gate: empty query allowed only if every selected source supports browse (or rutracker -C/-F given); otherwise prints help. Launcher torrent_search.sh fixed: onboarding only when $# -eq 0 (old case "${1:-}" swallowed an empty first arg).
- YTS API (movies-api.accel.li) quirks: limit>50 clamps to 20 movies; only limit=50 is a full page → yts.py now paginates internally (PAGE_SIZE 50, MAX_PAGES 20, 0.35s delay, window=rows, dedupe (movie_id, torrent_hash), past-the-end = HTTP 200 with `movies` key ABSENT, movie_count reliable; `page` kwarg shifts the loop for standalone callers). sort_by: download_count/rating/year/title honored; seeds silently ignored (date fallback). Mirror strips movie-level seeds/download_count.
- eztvx browse: RSS is a hard 30-item buffer, ALL params ignored (limit/page/quality/category); /releases/ HTML is Cloudflare-403 (fallback effectively dead); beta API GET /api/get-torrents (limit 1-100, page 1-100) works from this host, pages cleanly, seeds populated, imdb_id+hash+magnet_url per row → now the primary browse source (RSS/HTML kept as fallbacks). imdb_id enables a future --group-series (series-level popularity, TV's download_count equivalent).
- CLI changes recap: sources parsed above the gate; --yts-sort flag (warns if yts not selected); per-source fetch cap now max(page*limit, rt_pages*50); saved-session label "<sources> browse"; epilog + HowToUse examples.
