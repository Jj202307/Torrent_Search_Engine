"""Maxitorrent.net torrent scraper (login required)."""
...
BASE_URL = SITE_URLS["maxitorrent"]
...
class MaxitorrentScraper(BaseScraper):
    source = Source.MAXITORRENT
    ...
    async def _login(self) -> bool:
        ...
        try:
            ...
            post_url = _abs(BASE_URL, action or "takelogin.php")
            resp = await client.post(post_url, data=data)
            if resp.status_code >= 400:
                return False
            if client.cookies.get("uid") and client.cookies.get("pass"):
                self._logged_in = True
            else:
    ...
    def _parse_results(self, html: str, category: str = "", base_url: str | None = None) -> list[SearchResult]:
        ...
        results = []
        seen = set()
        for row in soup.select("table tr, .result tr, .data tr, tr"):
            link = row.select_one("a[href*='torrent'], a[href*='details'], a[href*='?id=']")
            if not link:
                continue
            href = link.get("href", "")
            if not href or href.startswith(("javascript", "#", "magnet:")):
                continue
            ...
            size_bytes = parse_size(row.get_text(" ", strip=True))
            seeders = leechers = 0
            if size_idx >= 0:
