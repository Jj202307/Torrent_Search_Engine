"""Zamunda.net torrent scraper (login required)."""
...
BASE_URL = SITE_URLS["zamunda"]
LOGIN_PAGE = f"{BASE_URL}/login.php"
LOGIN_POST = f"{BASE_URL}/takelogin.php"
...
class ZamundaScraper(BaseScraper):
    source = Source.ZAMUNDA
    ...
    def _parse_results(self, html: str, category: str = "", base_url: str | None = None) -> list[SearchResult]:
        ...
        for row in soup.select("tr"):
            ...
            href = link.get("href", "")
            details_url = _abs(base, href)
            dl = row.select_one("a[href*='download.php'], a[href*='/download']")
            torrent_url = details_url
            if dl:
                torrent_url = _abs(base, dl.get("href", ""))
            size_idx = next(
